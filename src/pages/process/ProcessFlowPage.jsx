...
const flowStages = [
  {
    ...
    output: 'ERP starts with clean masters. Customer, supplier, item, store, rack, bin, item group and process masters must be ready before transactions.',
  },
  {
    no: '02',
    title: 'Engineering Master Setup',
    department: 'Planning / Engineering',
    tone: 'amber',
    icon: GitBranch,
    steps: [
      { label: 'Master BOM', value: `${sampleData.bom} for ${sampleData.item}`, path: '/process/bom' },
      { label: 'Component Mapping', value: `${sampleData.rawMaterial} x 5`, path: '/process/bom/new' },
      { label: 'Route Sheet', value: 'RS-0001 operation flow', path: '/process/routesheet' },
      { label: 'Release Check', value: 'BOM + Route Sheet Active', path: '/settings/flow' },
    ],
    output: 'Master BOM and Route Sheet are setup before Sales Order MRP/Work Order. Without active BOM and route sheet, WO should not be released.',
  },
  {
    no: '03',
  ...
  {
    no: '04',
    title: 'MRP From Sales Order',
    department: 'Planning',
    tone: 'violet',
    icon: ClipboardList,
    steps: [
      { label: 'SO Demand', value: `${sampleData.so} x FG Qty`, path: '/process/so' },
      { label: 'BOM Explosion', value: 'FG -> RM/components', path: '/process/bom' },
      { label: 'MRP Run', value: sampleData.mrp, path: '/process/mrp' },
      { label: 'Stock Check', value: 'Available vs Required', path: '/reports/inventory' },
    ],
    output: 'MRP must explode the active Master BOM against Sales Order quantity, check accepted stock, and create PR only for shortage materials.',
  },
  {
    no: '05',
  ...
  },
  {
    ...
    output: 'WO is generated only with active BOM and Route Sheet. Production consumes accepted raw material and records output traceability.',
  },
  {
    no: '08',
...
const decisionRules = [
  {
    ...
    question: 'Active Master BOM and Route Sheet available for FG item?',
    yes: 'Sales Order can move to MRP and Work Order generation.',
    no: 'Create/release BOM and Route Sheet before production planning.',
  },
  {
    title: 'MRP Decision',
...
const controls = [
  ...
  'Master BOM maps FG item to required RM/components before MRP.',
  'Route Sheet maps production process sequence before Work Order release.',
  'MRP explodes BOM, checks accepted stock and can generate PR for shortage.',
...
function FlowImage() {
  const nodes = [
    ...
    ['Master BOM', 'FG -> RM/components', 250, 72, '#92400e'],
    ['Route Sheet', 'Process sequence', 480, 72, '#92400e'],
    ['CRM / SO', 'Customer demand', 710, 72, '#0e7490'],
    ['MRP', 'BOM explosion + stock', 940, 72, '#5b21b6'],
    ['PR + PO', 'Shortage purchase', 250, 238, '#991b1b'],
    ['Inward', 'POI-0001 location', 480, 238, '#0f766e'],
    ['Quality', 'Accepted / Rejected / Hold', 710, 238, '#0f766e'],
    ['Stock', 'Usable stock report', 940, 238, '#166534'],
